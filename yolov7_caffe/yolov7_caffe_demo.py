...
net_file = './yolov7.prototxt'
caffe_model = './yolov7.caffemodel'
...
def postprocess(out, img_h, img_w):
    ...
    output = []
    output.append(out['conv_blob90'].reshape((-1)))
    output.append(out['conv_blob91'].reshape((-1)))
    output.append(out['conv_blob92'].reshape((-1)))
...
def preprocess(src):
    #img = cv2.resize(src, (input_imgW, input_imgH), interpolation=cv2.INTER_LINEAR)
    img, ratio, (dw, dh) = letterbox(src, new_shape=(input_imgW, input_imgW))
